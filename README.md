...
**This performance analysis is from the angle of each streaming data where `n` is the amount of the valid donations already stored in memory.** The bottleneck performance of the program is adding donation to `Percentile` which is O(log(n)). Whereas, the worst case performance is O(log<sup>2</sup>(n)) and is only theoretically possible. Two key analyses are discussed in detail below.
...
The best way to understand object-oriented programming paradigm is looking at the UML class diagram and sequence diagram as shown here. Both diagrams were drawn using [astah](http://astah.net/). The class diagram was generated and add slight  modifications; the sequence diagram was drawn from scratch.
